...
Website editing is supported through the GitHub API. Nothing is sent back to a player or desktop media library yet.
...
device** is enabled by default. Close setup, browse episodes, and tap **Edit**.
...
## Progress storage and future desktop sync
...
**The desktop does not yet import website progress.** Website overrides remain
authoritative for their episodes until future reconciliation is implemented;
a newer desktop upload alone must not silently discard them.
...
file. Future desktop sync should introduce shared stable IDs and migrate this
format rather than assume upload time is a per-episode modification time.
